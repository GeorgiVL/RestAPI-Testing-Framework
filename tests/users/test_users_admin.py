...
def test_admin_delete_an_user(login_as_admin):
    ...
    user_deletion = Users().delete_an_user(APP_URL, login_as_admin, user_id)
    assert user_deletion.ok


def test_admin_creation(login_as_admin):
    admin_creation = Users().create_an_user(APP_URL, login_as_admin, new_username_admin, new_password_admin,
                                            new_user_role_admin)
...
def test_admin_delete_admin(login_as_admin):
    user_id = Users().get_user_id(APP_URL, login_as_admin)
    admin_deletion = Users().delete_an_user(APP_URL, login_as_admin, user_id)
